...
def sample_trajectory(env, policy, max_path_length, render=False):
    ...
    while True:
        ...
        ob_tensor = ptu.from_numpy(ob[None])  # shape (1, ob_dim)
        action_distribution = policy(ob_tensor)  # policy's forward returns a distribution
        ac = ptu.to_numpy(action_distribution.sample())  # shape (1, ac_dim) # HINT: this is a numpy array
        ac = ac[0]
